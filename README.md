...
2. **点 _注册_，选择 _管理ACG和收藏与收视进度，分享交流_ 这一项**
3. **填好各项基本信息和验证码**
...
1. **Bangumi是什么**
    ...
    > Bangumi提供了番剧索引、番剧收藏、追番进等功能，Ani可以将你的观看记录同步至 Bangumi。
2. **浏览器提示网站被屏蔽或禁止访问**
    > 更换浏览器（如 Kiwi/Yandex/Firefox/Chrome/Edge）   
    > 请在系统设置中更换默认浏览器
3. **验证码错误，请返回重试**
    > 返回后点击验证码刷新，输入新验证码登录
4. **无法收到邮箱验证码**
...
7. **Animeko中网页登录成功后没有反应**
    > 可以尝试使用 _令牌登录_   
    > 入口在 _登录Bangumi账号_ 中 _网页登录成功后没有反应_ 折叠项

> [!IMPORTANT]
> - 确保 Animeko 中 登录页面 和 授权页面 显示为移动端视图   
> - 若为桌面端视图，请点右上角，在菜单中取消勾选 _桌面版网站_   
> - 桌面版视图会导致授权失败   
> - 登录成功后等弹出 _此网站想打开Animeko应用_ 提示，点 _打开_
...
> [!IMPORTANT]
> - 数据来源于安卓端 4.10.1 版本   
> - 低于 4.10.1 版本可能会有部分条目搜索出现“未知错误”   
> - 版本迭代比较快，请尽量保持最新版本
> - 强烈建议初次使用且懒得进设置项的新手看完，避免不必要的麻烦他人
<details>
...
### **实战: 开发者工具 - Emby数据源编写（通用）**
1. **在浏览器登录Emby服务器，非管理员账号也可用**
2. **进 _设置_ 页面打开 _用户配置_，打开浏览器的 _开发者工具_**
3. **切换到 _网络_ 选项卡，选择 _过滤_ 下面一行的 _文档_**
4. **回到Emby，刷新一下再回到 _开发者工具_**
5. **点 _index.html_，点右侧的 _启动器_**
6. **从启动器下的链接找到 _api_key=_ 和 _UserId=_ 后的参数，直到 _&_**
7. **打开Animeko，进 _数据源管理_，点 _+_ 选择 _Emby_**
8. **在弹出窗口依次填入 _服务器地址、User Id、API Key_ 点 _保存_**

### **实战: 抓包工具 - Emby数据源编写（通用）**
1. **在浏览器登录Emby服务器，非管理员账号也可用**
2. **进 _设置_ 页面打开 _用户配置_**
3. **启动 _ProxyPin_ 应用，点右上角，选择 _应用白名单_**
4. **点右上角 _+_，选择你使用的浏览器**
5. **返回应用主页，点右下角按钮启动抓包**
6. **返回浏览器，刷新Emby _用户配置_ 页面**
7. **返回 _ProxyPin_ 应用，在顶栏输入 _api_key_，点击任意项**
8. **复制 _api_key=_ 后的参数，直到 _&_**
9. **返回应用主页，在顶栏输入 _userid_，点击任意项**
10. **复制 _UserId=_ 后的参数，直到 _&_**
11. **打开Animeko，进 _数据源管理_，点 _+_ 选择 _Emby_**
12. **在弹出窗口依次填入 _服务器地址、User Id、API Key_ 点 _保存_**

### **实战: 控制台 - Emby数据源编写（仅管理员）**
1. **在浏览器用管理员账号登录Emby服务器**
2. **在控制台左侧菜单中点 _用户_**
3. **选择要获取User ID的账号，复制地址栏 _UserId=_ 后的参数，直到 _&_**
4. **在控制台左侧菜单中点 _高级_，在顶栏点 _安全_**
5. **点 _Api 密钥_ 旁的 _添加_，为API密钥输入一个名称以便于识别**
6. **复制创建的Api密钥**
7. **打开Animeko，进 _数据源管理_，点 _+_ 选择 _Emby_**
8. **在弹出窗口依次填入 _服务器地址、User Id、API Key_ 点 _保存_**
